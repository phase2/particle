 ...
 * @param {String} config.namespaces.config - Path to config yaml to read/write
 * @param {String} config.namespaces.atKey - Lodash set string of path in object
...
module.exports = function namespaceWriter(namespaces, config) {
  ...
  return done => {
    ...
    const relNamespaces = Object.keys(namespaces).reduce((acc, cat) => {
      acc[cat] = {
        // _.compact() removes falsy values from the transform()'d array
        paths: _.compact(
          [].concat(

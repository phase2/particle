 ...
 * @returns {*}
 */
module.exports = function GulpTwigNamespaces(opt) {
  // Options from caller
  const options = opt || {};
  ...
  function bufferContents(vinylFile, encoding, callback) {
    ...
    // cwd: /Users/UserName/dev/pattern-lab-starter
    // base: /Users/UserName/dev/pattern-lab-starter/source/_patterns/
    // path: /Users/UserName/dev/pattern-lab-starter/source/_patterns/00-protons/demo/grid/_grid.twig
    //
    ...
    const bufferFiles = _.map(options.outputs, output => {
      ...
      const namespaces = _.reduce(
        ...
        (result, namePath, name) => {
          // Paths per namespace are unique to the output yaml files they will go into
